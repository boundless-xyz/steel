...
- Guest examples now enable the `bn` accelerated revm precompile and pin patched dependency versions (`k256`, `substrate-bn`, `tiny-keccak`) to match their risc0 forks. `blst` and `c-kzg` precompiles have been removed pending risc0 forks compatible with revm 36.
