...
//!
//! Patched version of `op-alloy-network` 0.24.0 that removes the conflicting
//! `NetworkWallet<Optimism>` impl for `EthereumWallet`, which is now covered by
//! the blanket impl in `alloy-network` 1.8+.
//!
//! See: <https://github.com/ethereum-optimism/optimism/pull/19756>
//! Remove this patch once a new `op-alloy-network` is published with the fix.
...
impl Network for Optimism {
    ...
    type ReceiptEnvelope = alloy_consensus::ReceiptWithBloom<op_alloy_consensus::OpReceipt>;
